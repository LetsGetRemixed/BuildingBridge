# Firebase App Hosting Secrets Setup Commands

This document contains all the CLI commands to set Firebase App Hosting secrets for the Building Bridge Foundation project.

**Note:** Make sure you're logged in to Firebase CLI first:
```bash
firebase login
```

## MongoDB & Auth Secrets
...
```powershell
.\set-firebase-secrets.ps1
```

**For Linux/Mac:**
```bash
chmod +x set-firebase-secrets.sh
./set-firebase-secrets.sh
```

## Important Notes

- **NEXT_PUBLIC_*** variables are public and will be exposed in the client bundle. These can still be set as secrets for consistency, but they're not truly secret.
- The `GOOGLE_APPLICATION_CREDENTIALS` variable is not included as we're using individual service account variables instead.
- Make sure to set these secrets before deploying to Firebase App Hosting.
